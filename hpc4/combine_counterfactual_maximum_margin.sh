#!/bin/bash
# Script to combine all counterfactual maximum margin results
# Usage: bash combine_counterfactual_maximum_margin.sh
...
result_dir <- "output/multihome/counterfactual/maximum_margin"
pattern <- "counterfactual_maximum_margin_t\\d+_j\\d+\\.rds$"
...
output_file <- file.path(result_dir, "counterfactual_maximum_margin_combined.rds")
...
if [ $? -eq 0 ]; then
  ...
  echo "Output: output/multihome/counterfactual/maximum_margin/counterfactual_maximum_margin_combined.rds"
  echo "=========================================="
else
  echo ""
