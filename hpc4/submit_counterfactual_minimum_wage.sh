#!/bin/bash
# Script to submit counterfactual minimum wage jobs
# Run this directly from the login node (not as a SLURM job)
# Usage: bash submit_counterfactual_minimum_wage.sh
...
if [ ! -f "hpc4/run_counterfactual_minimum_wage.sh" ]; then
  echo "ERROR: Cannot find hpc4/run_counterfactual_minimum_wage.sh" >&2
...
submit_job() {
  ...
  local submit_output
  submit_output=$(sbatch --job-name=min_wage_t${t}_j${j} \
    hpc4/run_counterfactual_minimum_wage.sh $t $j 2>&1)
  local submit_exit=$?
