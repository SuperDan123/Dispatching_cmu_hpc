#!/bin/bash
#SBATCH --job-name=max_margin        # create a short name for your job (override with --job-name when submitting)
...
#
# Usage: sbatch --job-name=max_margin_t1_j1 run_counterfactual_maximum_margin.sh <t> <j>
#   t: market index (1-5)
#   j: zone index (max 75 for t=1,2; max 69 for t=3; max 72 for t=4,5)
# Example: sbatch --job-name=max_margin_t1_j1 run_counterfactual_maximum_margin.sh 1 1
...
if [ -z "$T" ] || [ -z "$J" ]; then
  ...
  echo "Usage: sbatch run_counterfactual_maximum_margin.sh <t> <j>"
...
mkdir -p output/multihome/counterfactual/maximum_margin

# Run R script with market and zone indices as arguments
echo "Starting R script execution..."
Rscript main/multihome/06_counterfactual/counterfactual_maximum_margin.R $T $J \
  > output/multihome/counterfactual/maximum_margin/log_t${T}_j${J}.out \
  2> output/multihome/counterfactual/maximum_margin/log_t${T}_j${J}.err
RSCRIPT_EXIT_CODE=$?
...
if [ $RSCRIPT_EXIT_CODE -ne 0 ]; then
  ...
  echo "  output/multihome/counterfactual/maximum_margin/log_t${T}_j${J}.out" >&2
  echo "  output/multihome/counterfactual/maximum_margin/log_t${T}_j${J}.err" >&2
