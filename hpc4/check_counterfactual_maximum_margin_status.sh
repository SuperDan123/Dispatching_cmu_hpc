#!/bin/bash
# Script to check the status of all counterfactual maximum margin jobs
# Usage: bash check_counterfactual_maximum_margin_status.sh
...
for t in {1..5}; do
  ...
  for j in $(seq 1 $MAX_J); do
    ...
    log_err="output/multihome/counterfactual/maximum_margin/log_t${t}_j${j}.err"
    
    if [ -f "$output_file" ]; then
      COMPLETED=$((COMPLETED + 1))
    else
      # Check if job is in queue
      job_name="max_margin_t${t}_j${j}"
